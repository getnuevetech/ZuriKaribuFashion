...
const DEFAULT_PRODUCT_SKU_SETTINGS: ProductSkuSettings = {
  enabled: true,
  serialLength: 6,
  prefixes: {
    READY_TO_WEAR: 'R',
...
export default function AdminProducts() {
  ...
  const [skuSettings, setSkuSettings] = useState<ProductSkuSettings>({ ...DEFAULT_PRODUCT_SKU_SETTINGS });
  ...
  const fetchProductSkuSettings = async () => {
    try {
      setSkuLoading(true);
      const response = await api.admin.getProductSkuSettings();
      const row = response?.data || {};
      setSkuSettings({
        enabled: row.enabled !== false,
        serialLength: clamp(row.serialLength, 6, 4, 12),
        prefixes: {
          READY_TO_WEAR: String(row.prefixes?.READY_TO_WEAR || 'R').trim().slice(0, 4) || 'R',
          FABRIC: String(row.prefixes?.FABRIC || 'F').trim().slice(0, 4) || 'F',
          DESIGN: String(row.prefixes?.DESIGN || 'C').trim().slice(0, 4) || 'C',
        },
      });
    } catch {
      setSkuSettings({ ...DEFAULT_PRODUCT_SKU_SETTINGS });
    } finally {
      setSkuLoading(false);
    }
  };

  const saveProductSkuSettings = async () => {
    try {
      setSkuSaving(true);
      await api.admin.updateProductSkuSettings(skuSettings);
      setProductViewMessageType('success');
      setProductViewMessage('Product SKU settings saved.');
    } catch (error) {
      setProductViewMessageType('error');
      setProductViewMessage(formatApiError(error, 'Unable to save Product SKU settings.'));
    } finally {
      setSkuSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-white p-4">
        ...
        <div className="mb-4 rounded-lg border bg-gray-50 p-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-sm font-semibold text-gray-900">Autogenerated SKU</p>
            <Button
              type="button"
              size="sm"
              variant="outline"
              onClick={() => void saveProductSkuSettings()}
              disabled={skuSaving || skuLoading}
            >
              {skuSaving ? 'Saving...' : 'Save SKU Settings'}
            </Button>
          </div>
          <p className="mb-3 text-xs text-gray-500">
            Configure automatic SKU format for RTW, FTB, and CTW product creation.
          </p>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            <label className="flex items-center gap-2 text-xs text-gray-700">
              <input
                type="checkbox"
                checked={skuSettings.enabled !== false}
                onChange={(event) => setSkuSettings((prev) => ({ ...prev, enabled: event.target.checked }))}
              />
              Enable SKU generator
            </label>
            <label className="text-xs text-gray-700">
              Serial length
              <input
                ...
                min={4}
                max={12}
                value={skuSettings.serialLength}
                onChange={(event) =>
                  setSkuSettings((prev) => ({
                    ...prev,
                    serialLength: clamp(event.target.value, prev.serialLength, 4, 12),
                  }))
                }
                className="mt-1 w-full rounded border px-2 py-1.5 text-xs"
              />
            ...
            <label className="text-xs text-gray-700">
              RTW prefix
              <input
                value={skuSettings.prefixes.READY_TO_WEAR}
                onChange={(event) =>
                  setSkuSettings((prev) => ({
                    ...prev,
                    prefixes: {
                      ...prev.prefixes,
                      READY_TO_WEAR: String(event.target.value || '')
                        .toUpperCase()
                        .replace(/[^A-Z0-9]/g, '')
                        .slice(0, 4),
                    },
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="text-xs text-gray-700">
                FTB prefix
                <input
                  value={skuSettings.prefixes.FABRIC}
                  onChange={(event) =>
                    setSkuSettings((prev) => ({
                      ...prev,
                      prefixes: {
                        ...prev.prefixes,
                        FABRIC: String(event.target.value || '')
                          .toUpperCase()
                          .replace(/[^A-Z0-9]/g, '')
                          .slice(0, 4),
                      },
              ...
              <label className="text-xs text-gray-700">
                CTW prefix
                <input
                  value={skuSettings.prefixes.DESIGN}
                  onChange={(event) =>
                    setSkuSettings((prev) => ({
                      ...prev,
                      prefixes: {
                        ...prev.prefixes,
                        DESIGN: String(event.target.value || '')
                          .toUpperCase()
                          .replace(/[^A-Z0-9]/g, '')
                          .slice(0, 4),
                      },
            ...
            <tbody>
              {filteredProducts.map((product) => (
                <tr key={product.id} className="border-b last:border-0 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 font-mono text-xs text-gray-500">{String(product.sku || '—')}</td>
