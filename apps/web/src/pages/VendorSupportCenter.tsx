...
export default function VendorSupportCenterPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.helpCenter.getPublic('VENDOR');
  ...
  const startWhatsApp = async (mode: 'CHAT' | 'CALL') => {
    ...
    try {
      const response = await api.customerService.startWhatsAppSession({
        ...
        message:
          mode === 'CALL'
            ? 'Seller/Designer initiated WhatsApp call request.'
            : 'Seller/Designer initiated WhatsApp chat with admin support.',
      });
      if (response.success && response.data?.eventLink) {
        window.open(response.data.eventLink, '_blank', 'noopener,noreferrer');
      }
    } catch (error: any) {
      setWhatsAppMessage(error?.response?.data?.message || 'Unable to launch WhatsApp support right now.');
    } finally {
    ...
    return (
      <div className="mx-auto max-w-5xl px-4 py-16">
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">
          Failed to load Seller/Designer support content. Please try again.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-10 sm:px-6">
      <section className="rounded-2xl bg-gray-900 px-6 py-10 text-white sm:px-8">
        ...
        {canInitiateVendorWhatsApp ? (
          <div className="mt-5 flex flex-wrap gap-2">
            <Button onClick={() => void startWhatsApp('CHAT')} disabled={startingWhatsApp}>
              <MessageCircle className="mr-2 h-4 w-4" />
              Chat Admin on WhatsApp
            </Button>
            <Button variant="outline" onClick={() => void startWhatsApp('CALL')} disabled={startingWhatsApp}>
              <Phone className="mr-2 h-4 w-4" />
              Call Admin on WhatsApp
            </Button>
          </div>
        ) : null}
        {whatsAppMessage ? <p className="mt-3 text-sm text-red-200">{whatsAppMessage}</p> : null}
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-4 rounded-2xl border bg-white p-4 sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Vendor FAQs</h2>
          </div>
        ...
        <div className="space-y-4 rounded-2xl border bg-white p-4 sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Vendor Support Contacts</h2>
          </div>
