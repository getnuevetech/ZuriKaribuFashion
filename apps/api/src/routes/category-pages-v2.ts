  ...
  .object({
    ...
    productCard: z
      .object({
        ...
        fieldOrder: z.array(z.enum(['DESIGNER_NAME', 'PRODUCT_NAME', 'SHORT_DESCRIPTION', 'PRICE'])).max(4).optional(),
        imageAspectRatio: z.enum(['3:4', '1:1']).optional(),
        textGap: z.number().int().min(0).max(24).optional(),
        contentPaddingX: z.number().int().min(0).max(40).optional(),
        contentPaddingY: z.number().int().min(0).max(40).optional(),
        designerNameEnabled: z.boolean().optional(),
        designerNameFontSize: z.number().int().min(8).max(72).optional(),
        designerNameColor: z.string().trim().max(40).optional(),
        productNameEnabled: z.boolean().optional(),
        ...
        shortDescriptionWordLimit: z.number().int().min(4).max(24).optional(),
        ...
        labelPosition: z.literal('TOP_LEFT').optional(),
        likesEnabled: z.boolean().optional(),
        likesSize: z.number().int().min(8).max(72).optional(),
        likesColor: z.string().trim().max(40).optional(),
        likesActiveColor: z.string().trim().max(40).optional(),
        likesPosition: z.literal('TOP_RIGHT').optional(),
        countryIconEnabled: z.boolean().optional(),
        countryIconSize: z.number().int().min(8).max(96).optional(),
        countryIconPosition: z.literal('BOTTOM_RIGHT').optional(),
      })
      .optional(),
    detailView: z
      .object({
        titleFontSize: z.number().int().min(18).max(96).optional(),
        titleColor: z.string().trim().max(40).optional(),
        ownerFontSize: z.number().int().min(10).max(42).optional(),
        ownerColor: z.string().trim().max(40).optional(),
        priceLabelColor: z.string().trim().max(40).optional(),
        priceValueFontSize: z.number().int().min(18).max(96).optional(),
        priceValueColor: z.string().trim().max(40).optional(),
        descriptionFontSize: z.number().int().min(12).max(36).optional(),
        descriptionColor: z.string().trim().max(40).optional(),
        specLabelColor: z.string().trim().max(40).optional(),
        specValueColor: z.string().trim().max(40).optional(),
        tabOrder: z.array(z.enum(['DETAILS', 'SPECS', 'REVIEWS'])).max(3).optional(),
        defaultTab: z.enum(['DETAILS', 'SPECS', 'REVIEWS']).optional(),
